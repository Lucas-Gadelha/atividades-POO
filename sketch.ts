...
class Lapiseira {
  ...
  setGrafite(grafite: Grafite): boolean{
    ...
    if(this.grafite != null && this.extra.length >= this.qtd){
      console.log("tambor da lapiseira cheio!");
      return false;
    }
  ...
  escrever(folhas: number): boolean{
    ...
    let total = this.grafite.tamanho / this.grafite.consumo();
    ...
    if(gasto <= this.grafite.tamanho){
      ...
      let restante = gasto - this.grafite.tamanho / this.grafite.consumo();
      ...
      // let escrito = this.grafite.tamanho / this.grafite.consumo();
...
let graf2 = new Grafite(0.5, "2B", 10);
